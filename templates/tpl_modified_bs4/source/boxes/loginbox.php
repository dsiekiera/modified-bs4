...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_login.html', $cache_id) || !$cache) {
  ...
  if (!isset($_SESSION['customer_id'])) {
    ...
    $box_smarty->assign('FIELD_EMAIL', xtc_draw_input_field('email_address', '', 'class="form-control" maxlength="50"'));
    $box_smarty->assign('FIELD_PWD', xtc_draw_password_field('password', '', 'class="form-control" maxlength="30"'));
    $box_smarty->assign('BUTTON', '<button class="btn btn-primary" role="button" type="submit">'.IMAGE_BUTTON_LOGIN.'</button>');
